...
app.get('/cf-admin/colleges', function(req,res) {
	collegeModel.find({}).exec(function(err, colleges){
		var college = [];

		colleges.forEach(function(document){
			college.push(document.toObject());
			//console.log(college);
		});
...
app.get('/cf-admin/reviews', function(req,res) {
	reviewModel.find({}).exec(function(err,result){
		var reviews = [];
		var profs = [];
		var users = [];
		...
		result.forEach(function(data){
			reviews.push({
				revUser: data.studentId,
				revProf: data.profNumber,
				revCourse: data.profCourse,
				revContent: data.reviewContent
			});
			//console.log(reviews);
		});
...
app.get('/cf-admin/professors', function(req,res) {
	professorModel.find({}).exec(function(err, prof){
		collegeModel.find({}).exec(function(err, college){
			var professor = [];
			var colleges = [];

			
			prof.forEach(function(document){
			professor.push(document.toObject());
			//console.log(professor);
			});

			college.forEach(function(document){
				colleges.push(document.toObject());
				//console.log(professor);
			});
...
app.get('/guide', function(req,res) {
	res.render('guide',{
		title: 'Comm Forum Guidelines',
		jumbotronImage: '/assets/headers/home_header.jpg',
		jumbotronHeader: 'User Panel',
		jumbotronMessage: 'The DLSU Community Forum have implemented guidelines and conditons in which all users of this website are expected to strictly follow these following guidelines',
		jumbotronLink: '/',
		jumbotronBtn: 'Back to Home'
	});
});
